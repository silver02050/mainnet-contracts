import FungibleToken from "../0xf233dcee88fe0abe/FungibleToken.cdc"
 ...
 pub contract PlayTodayRND: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance {
        ...
        destroy() {
            PlayTodayRND.totalSupply = PlayTodayRND.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        //
        pub fun mintTokens(amount: UFix64): @PlayTodayRND.Vault {
            pre {
    ...
    init() {
        ...
        self.account.save(<-admin, to: /storage/PlayTodayRNDAdmin)

        // mint tokens
        let tokenVault <- minter.mintTokens(amount: self.totalSupply)
        self.account.save(<-tokenVault, to: /storage/ptrndVault)
        destroy minter

        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        self.account.link<&PlayTodayRND.Vault{FungibleToken.Balance}>(
            /public/ptrndBalance,
            target: /storage/ptrndVault
        )

        // Create a public capability to the stored Vault that only exposes
        // the `deposit` method through the `Receiver` interface
        self.account.link<&{FungibleToken.Receiver}>(
            /public/ptrndReceiver,
            target: /storage/ptrndVault
        )
