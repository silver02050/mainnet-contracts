...
pub contract LCubeTickets: NonFungibleToken {
    ...
    pub resource Event {
        ...
        pub fun addTicketType(ticketTypeID: UInt32) {
            pre {
                LCubeTickets.ticketTypeDatas[ticketTypeID] != nil: "Cannot add the TicketType to Event: TicketType doesn't exist."
        ...
        pub fun mintTicket(ticketTypeID: UInt32, metadata: String?): @NFT {
            ...
            // Mint the new ticket
            let newTicket: @NFT <- create NFT(serialNumber: numInTicketType + 1,
    ...
    pub struct QueryEventData {
        ...
        init(eventID: UInt32) {
            pre {
                LCubeTickets.events[eventID] != nil: "The event with the provided ID does not exist"
            }

            let event = (&LCubeTickets.events[eventID] as &Event?)!
    ...
    //
    pub struct LCubeTicketMetadataView {
        ...
        pub fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<LCubeTicketMetadataView>():
                    return LCubeTicketMetadataView(
                        eventName: LCubeTickets.getEventName(eventID: self.data.eventID),
                        ticketTypeName: LCubeTickets.getTicketTypeName(ticketTypeID: self.data.ticketTypeID),
                        venue: LCubeTickets.getEventVenue(eventID: self.data.eventID),
                        startDate: LCubeTickets.getEventStartDate(eventID: self.data.eventID),
                        serialNumber: self.data.serialNumber,
                        ticketTypeID: self.data.ticketTypeID,
                        eventID: self.data.eventID,
                        numTicketsInTicketType: LCubeTickets.getNumTicketsInTicketType(eventID: self.data.eventID, ticketTypeID: self.data.ticketTypeID)
                    )
            }
    ...
    pub resource Admin {
        ...
        pub fun createEvent(name: String, startTime: String, venue: String, eventType: String, timeZone : Int, gateTime: String, image: String?, description: String?): UInt32 {
        ...
        pub fun borrowEvent(eventID: UInt32): &Event {
            pre {
                LCubeTickets.events[eventID] != nil: "Cannot borrow Event: Event doesn't exist"
            }
    ...
    pub resource interface LCubeTicketsCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun batchDeposit(tokens: @NonFungibleToken.Collection)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowTicket(id: UInt64): &LCubeTickets.NFT? {
    ...
    //
    pub resource Collection: LCubeTicketsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection { 
    ...
    pub fun getAllTicketTypes(): [LCubeTickets.TicketType] {
    ...
    init() {
        ...
        self.account.save<@Collection>(<- create Collection(), to: /storage/LCubeTicketsCollection)

        // Create a public capability for the Collection
        self.account.link<&{LCubeTicketsCollectionPublic}>(/public/LCubeTicketsCollection, target: /storage/LCubeTicketsCollection)

        // Put the Minter in storage
        self.account.save<@Admin>(<- create Admin(), to: /storage/LCubeTicketAdmin)
