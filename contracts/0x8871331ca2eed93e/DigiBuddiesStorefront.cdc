...
pub contract DigiBuddiesStorefront {
    
    pub event DigiBuddiesStorefrontInitialized()
    ...
    pub resource Listing: ListingPublic {
        ...
        pub fun purchase(payment: @FungibleToken.Vault, collection: &NonFungibleToken.Collection{NonFungibleToken.Receiver}) {
            ...
            if(getAccount(buyerAddress).getCapability<&{NonFungibleToken.CollectionPublic}>(DigiBuddies.CollectionPublicPath).borrow() != nil){
                let driverzCollection:&{NonFungibleToken.CollectionPublic} = getAccount(buyerAddress).getCapability<&{NonFungibleToken.CollectionPublic}>(DigiBuddies.CollectionPublicPath).borrow()!
