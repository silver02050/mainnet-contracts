...
pub contract ZayTraderV2 {
    ...
    pub resource interface TradeOfferPublic {
        ...
        pub fun acceptTrade(
            ...
            feeCap: Capability<&FlowToken.Vault{FungibleToken.Provider}>,
    ...
    pub resource TradeOffer: TradeOfferPublic {
        ...
        pub fun acceptTrade(
            ...
            if (self.details.requestedAddress != nil) {
                ...
                assert(
                    signingAddress! == self.details.requestedAddress!,
                    message: "Requested trade account does not match provided account or expected signature not provided"
                )
                assert(
                    feeCap.address == self.details.requestedAddress!,
                    message: "Requested trade account does not match provided account given for fee payment"
                )
                let requestedCollection = getAccount(self.details.requestedAddress!).getCapability<&ZayTraderV2.TradeCollection{ZayTraderV2.TradeCollectionPublic}>(ZayTraderV2.CollectionPublicPath).borrow()!
                assert(
                    requestedCollection.getCurrentApprovingOfferID()! == self.uuid,
                    message: "Expected approving trade offer ID does not match UUID of this trade offer"
                )
            }
            ...
            if (signingAddress != nil && signedMessage != nil) {
                let signatureTimestamp = ZayVerifierV2.verifySignature(
                    acctAddress: signingAddress!,
                    message: signedMessage!,
                    keyIds: keyIds,
                    signatures: signatures,
                    signatureBlock: signatureBlock!,
                    intent: "Execute_Schwap_Account_Ownership_Verification_",
                    identifier: self.uuid.toString()
                )
    ...
    pub resource interface TradeCollectionManager {
        ...
        //
        pub fun setCurrentApprovingOfferID(tradeOfferResourceID: UInt64?)
    }
    ...
    pub resource interface TradeCollectionPublic {
        ...
        pub fun getCurrentApprovingOfferID(): UInt64?
    ...
    pub resource TradeCollection : TradeCollectionManager, TradeCollectionPublic {
        ...
        access(self) var currentApprovingOfferID: UInt64?
