...
pub contract CarClubDropStorefront {
    ...
    pub event CarClubDropStorefrontInitialized()
    ...
    pub resource Listing: ListingPublic {
        ...
        pub fun purchase(payment: @FungibleToken.Vault, collection: &NonFungibleToken.Collection{NonFungibleToken.Receiver}) {
            ...
            if(getAccount(buyerAddress).getCapability<&{NonFungibleToken.CollectionPublic}>(DriverzNFT.CollectionPublicPath).borrow() != nil){
                let driverzCollection:&{NonFungibleToken.CollectionPublic} = getAccount(buyerAddress).getCapability<&{NonFungibleToken.CollectionPublic}>(DriverzNFT.CollectionPublicPath).borrow()!
            ...
            if(payment.balance != finalSalePrice){
                panic("Payment vault does not contain requested price, Driverz owners = $77 and non-owners = $97")
            }
    ...
    init () {
        self.StorefrontStoragePath = /storage/CarClubDropStorefront
        self.StorefrontPublicPath = /public/CarClubDropStorefront

        emit CarClubDropStorefrontInitialized()
    }
}
