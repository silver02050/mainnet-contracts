...
access(all)
contract LOSTiNTurismo: NonFungibleToken{ 
	// -----------------------------------------------------------------------
	...
	struct ItemData{ 
		...
		init(name: String, description: String, thumbnail: String, thumbnailCID: String, thumbnailPathIPFS: String?, thumbnailMimeType: String, thumbnailHosting: String, mediaURL: String, mediaCID: String, mediaPathIPFS: String?, mimetype: String, mediaHosting: String, attributes:{ String: String}, rarity: String){ 
			self.merchantID = LOSTiNTurismo.merchantID
			self.name = name
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		init(name: String, description: String, thumbnail: String, thumbnailCID: String, thumbnailPathIPFS: String?, thumbnailMimeType: String, thumbnailHosting: String, mediaURL: String, mediaCID: String, mediaPathIPFS: String?, mimetype: String, mediaHosting: String, attributes:{ String: String}, rarity: String){ 
			LOSTiNTurismo						 // Increment the global Item IDs
						 .totalSupply = LOSTiNTurismo.totalSupply + 1 as UInt64
			self.id = LOSTiNTurismo.totalSupply
			...
			emit ItemMinted(itemID: self.id, merchantID: LOSTiNTurismo.merchantID, name: name)
		}
		...
		fun getMutation(): ItemData?{ 
			return LOSTiNTurismo.mutations[self.id]
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<MetadataViews.Display>():
					var thumbnail:{ MetadataViews.File} = MetadataViews.HTTPFile(url: self.data.thumbnail)
					if self.data.thumbnailHosting == "IPFS"{ 
						thumbnail = MetadataViews.IPFSFile(cid: self.data.thumbnailCID, path: self.data.thumbnailPathIPFS)
					}
					return MetadataViews.Display(name: self.data.name, description: self.data.description, thumbnail: thumbnail)
				case Type<MetadataViews.Editions>():
					let editionInfo = MetadataViews.Edition(name: self.data.name, number: UInt64(1), max: 1)
					let editionList: [MetadataViews.Edition] = [editionInfo]
					return MetadataViews.Editions(editionList)
				case Type<MetadataViews.Royalties>():
					let royaltiesDictionary = LOSTiNTurismo.royaltiesForSpecificPFP[self.id] ?? LOSTiNTurismo.defaultRoyalties
				...
				case Type<MetadataViews.ExternalURL>():
					return LOSTiNTurismo.ExternalURL
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: LOSTiNTurismo.CollectionStoragePath, publicPath: LOSTiNTurismo.CollectionPublicPath, publicCollection: Type<&LOSTiNTurismo.Collection>(), publicLinkedType: Type<&LOSTiNTurismo.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-LOSTiNTurismo.createEmptyCollection(nftType: Type<@LOSTiNTurismo.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					...
					return MetadataViews.NFTCollectionDisplay(name: "LOSTiNTurismo", description: LOSTiNTurismo.Description, externalURL: LOSTiNTurismo.ExternalURL, squareImage: LOSTiNTurismo.SquareImage, bannerImage: LOSTiNTurismo.BannerImage, socials: LOSTiNTurismo.Socials)
	...
	resource Mutator{ 
		...
		fun mutatePFP(tokenID: UInt64, name: String, description: String, thumbnail: String, thumbnailCID: String, thumbnailPathIPFS: String?, thumbnailMimeType: String, thumbnailHosting: String, mediaURL: String, mediaCID: String, mediaPathIPFS: String?, mimetype: String, mediaHosting: String, attributes:{ String: String}, rarity: String){ 
			pre{ 
				tokenID <= LOSTiNTurismo.totalSupply:
					"the tokenID does not exist"
			}
			if LOSTiNTurismo.lockedMutators[self.id] != true{ 
				...
				LOSTiNTurismo.mutations[tokenID] = mutation
	...
	resource Admin{ 
		...
		fun setExternalURL(url: String){ 
			LOSTiNTurismo.ExternalURL = MetadataViews.ExternalURL(url)
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun addSocial(key: String, url: String):{ String: MetadataViews.ExternalURL}{ 
			LOSTiNTurismo.Socials.insert(key: key, MetadataViews.ExternalURL(url))
			return LOSTiNTurismo.getSocials()
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun removeSocial(key: String):{ String: MetadataViews.ExternalURL}{ 
			LOSTiNTurismo.Socials.remove(key: key)
			return LOSTiNTurismo.getSocials()
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun setDescription(description: String){ 
			LOSTiNTurismo.Description = description
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun setSquareImage(url: String, mediaType: String){ 
			LOSTiNTurismo.SquareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: url), mediaType: mediaType)
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun setBannerImage(url: String, mediaType: String){ 
			LOSTiNTurismo.BannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: url), mediaType: mediaType)
		}
		...
		fun createNewAdmin(): @Admin{ 
			let newID = LOSTiNTurismo.nextAdminID
			// Increment the ID so that it isn't used again
			LOSTiNTurismo.nextAdminID = LOSTiNTurismo.nextAdminID + 1 as UInt32
		...
		fun createNewMutator(): @Mutator{ 
			let newID = LOSTiNTurismo.nextMutatorID
			// Increment the ID so that it isn't used again
			LOSTiNTurismo.nextMutatorID = LOSTiNTurismo.nextMutatorID + 1 as UInt32
		...
		fun lockMutator(id: UInt32): Int{ 
			LOSTiNTurismo.lockedMutators.insert(key: id, true)
			return LOSTiNTurismo.lockedMutators.length
		}
		
		// Unlocks a mutator
		access(TMP_ENTITLEMENT_OWNER)
		fun unlockMutator(id: UInt32): Int{ 
			LOSTiNTurismo.lockedMutators.remove(key: id)
			return LOSTiNTurismo.lockedMutators.length
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun setMerchantID(merchantID: UInt32): UInt32{ 
			LOSTiNTurismo.merchantID = merchantID
			return LOSTiNTurismo.merchantID
		}
		...
		fun addDefaultRoyalty(name: String, royalty: MetadataViews.Royalty, rate: UFix64){ 
			pre{ 
				LOSTiNTurismo.defaultRoyalties[name] == nil:
			...
			LOSTiNTurismo.defaultRoyalties[name] = royalty
		...
		fun changeDefaultRoyaltyRate(name: String, rate: UFix64){ 
			...
			let royalty = LOSTiNTurismo.defaultRoyalties[name]!
			let previousRate = royalty.cut
			let previousRecipientAddress = royalty.receiver
			LOSTiNTurismo.defaultRoyalties[name] = MetadataViews.Royalty(receiver: previousRecipientAddress, cut: UFix64(rate), description: "LOSTiNTurismo Royalties")
		...
		fun removeDefaultRoyalty(name: String){ 
			...
			LOSTiNTurismo.defaultRoyalties.remove(key: name)
		...
		fun addRoyaltyForPFP(tokenID: UInt64, name: String, royalty: MetadataViews.Royalty, rate: UFix64){ 
			...
			if LOSTiNTurismo.royaltiesForSpecificPFP.containsKey(tokenID) == false{ 
				let newEntry:{ String: MetadataViews.Royalty} ={} 
				newEntry.insert(key: name, royalty)
				(LOSTiNTurismo.royaltiesForSpecificPFP!).insert(key: tokenID, newEntry)
				emit RoyaltyForPFPAdded(tokenID: tokenID, name: name, rate: rate)
				return
			}
			// the TokenID already has an entry
			if (LOSTiNTurismo.royaltiesForSpecificPFP[tokenID]!).containsKey(name){ 
				// the entry already exists
				panic("The royalty with that name already exists")
			}
			(LOSTiNTurismo.royaltiesForSpecificPFP[tokenID]!).insert(key: name, royalty)
			emit RoyaltyForPFPAdded(tokenID: tokenID, name: name, rate: rate)
		}
		...
		fun changeRoyaltyRateForPFP(tokenID: UInt64, name: String, rate: UFix64){ 
			...
			let previousRoyalty: MetadataViews.Royalty = (LOSTiNTurismo.royaltiesForSpecificPFP[tokenID]!)[name]!
			let newRoyalty = MetadataViews.Royalty(receiver: previousRoyalty.receiver, cut: UFix64(rate), description: "LOSTiNTurismo Royalties")
			(LOSTiNTurismo.royaltiesForSpecificPFP[tokenID]!).insert(key: name, newRoyalty)
		...
		fun removeRoyaltyForPFP(tokenID: UInt64, name: String){ 
			(LOSTiNTurismo.royaltiesForSpecificPFP[tokenID]!).remove(key: name)
		...
		fun revertRoyaltyForPFPToDefault(tokenID: UInt64){ 
			LOSTiNTurismo.royaltiesForSpecificPFP.remove(key: tokenID)
	...
	access(all)
	resource interface LOSTiNTurismoCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowLOSTiNTurismo(id: UInt64): &LOSTiNTurismo.NFT?{ 
	...
	//
	access(all)
	resource Collection: LOSTiNTurismoCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		fun deposit(token: @{NonFungibleToken.NFT}): Void{ 
			...
			let token <- token as! @LOSTiNTurismo.NFT
			// Get the token's ID
			let id = token.id
		...
		// not any MintPFPs specific data. Please use borrowLOSTiNTurismos to 
		// read MintPFPs data.
		//
		access(all)
		...
		// borrowLOSTiNTurismo returns a borrowed reference to a MintPFPs
		...
		fun borrowLOSTiNTurismo(id: UInt64): &LOSTiNTurismo.NFT?{ 
			if self.ownedNFTs[id] != nil{ 
				...
				return ref as! &LOSTiNTurismo.NFT
			} else{ 
				return nil
			}
		}
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			let LOSTiNTurismoNFT = nft as! &LOSTiNTurismo.NFT
			return LOSTiNTurismoNFT as &{ViewResolver.Resolver}
		}
	...
	fun createEmptyCollection(nftType: Type): @{NonFungibleToken.Collection}{ 
		return <-create LOSTiNTurismo.Collection()
	}
	
	access(TMP_ENTITLEMENT_OWNER)
	fun createEmptyMintPFPsCollection(): @LOSTiNTurismo.Collection{ 
		return <-create LOSTiNTurismo.Collection()
	}
	...
	fun getSocials():{ String: MetadataViews.ExternalURL}{ 
		return LOSTiNTurismo.Socials
	}
	
	access(TMP_ENTITLEMENT_OWNER)
	fun getDescription(): String{ 
		return LOSTiNTurismo.Description
	}
	
	access(TMP_ENTITLEMENT_OWNER)
	fun getSquareImage(): MetadataViews.Media{ 
		return LOSTiNTurismo.SquareImage
	}
	
	access(TMP_ENTITLEMENT_OWNER)
	fun getBannerImage(): MetadataViews.Media{ 
		return LOSTiNTurismo.BannerImage
	}
	
	// Returns all of the locked mutator IDs
	access(TMP_ENTITLEMENT_OWNER)
	fun getLockedMutators():{ UInt32: Bool}{ 
		return LOSTiNTurismo.lockedMutators
	}
	...
	init(){ 
		...
		self.lockedMutators ={} 
		self.ExternalURL = MetadataViews.ExternalURL("https://lostin.com")
		self.Socials ={} 
		self.Description = "The LOST iN Turismo Community. A space where LOST iN readers can interact with each other and the brand. Participation is granted through the purchase of a unique token (NFT) depicting a remixed version of our iconic cover. This token then grants you special access as well as participation in our upcoming editions as we open up to our community. We hope you are as excited as us in this new chapter for our brand and welcome you to join us on this trip."
		self.SquareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://mint-store-metadata.s3.us-east-2.amazonaws.com/lostin/utility-thumbnails/logo_square(2).png"), mediaType: "image/png")
		self.BannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://mint-store-metadata.s3.us-east-2.amazonaws.com/lostin/utility-thumbnails/Vuurtoreneiland_Annelore-01.png"), mediaType: "image/png")
		self.CollectionStoragePath = /storage/LOSTiNTurismoCollection
		self.CollectionPublicPath = /public/LOSTiNTurismoCollection
		self.AdminStoragePath = /storage/LOSTiNTurismoAdmin
		self.MutatorStoragePath = /storage/LOSTiNTurismoMutator
		// Put a new Collection in storage
		self.account.storage.save<@Collection>(<-create Collection(), to: self.CollectionStoragePath)
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{LOSTiNTurismoCollectionPublic}>(self.CollectionStoragePath)
