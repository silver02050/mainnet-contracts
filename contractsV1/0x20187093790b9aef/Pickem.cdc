...
access(all)
contract Pickem: NonFungibleToken{ 
	// -----------------------------------------------------------------------
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		init(name: String, description: String, thumbnail: String, thumbnailCID: String, thumbnailPathIPFS: String?, thumbnailMimeType: String, thumbnailHosting: String, mediaURL: String, mediaCID: String, mediaPathIPFS: String?, mimetype: String, mediaHosting: String, attributes:{ String: String}, rarity: String){ 
			Pickem				  // Increment the global Item IDs
				  .totalSupply = Pickem.totalSupply + 1 as UInt64
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: Pickem.CollectionStoragePath, publicPath: Pickem.CollectionPublicPath, publicCollection: Type<&Pickem.Collection>(), publicLinkedType: Type<&Pickem.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-Pickem.createEmptyCollection(nftType: Type<@Pickem.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					let data = self.getData()
					let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: data.thumbnail), mediaType: data.thumbnailMimeType)
					return MetadataViews.NFTCollectionDisplay(name: "Pickem", description: Pickem.Description, externalURL: Pickem.ExternalURL, squareImage: Pickem.SquareImage, bannerImage: Pickem.BannerImage, socials: Pickem.Socials)
	...
	resource Admin{ 
		...
		fun changeDefaultRoyaltyRate(name: String, rate: UFix64){ 
			...
			Pickem.defaultRoyalties[name] = MetadataViews.Royalty(receiver: previousRecipientAddress, cut: UFix64(rate), description: "Pickem Royalties")
		...
		fun changeRoyaltyRateForPFP(tokenID: UInt64, name: String, rate: UFix64){ 
			...
			let newRoyalty = MetadataViews.Royalty(receiver: previousRoyalty.receiver, cut: UFix64(rate), description: "Pickem Royalties")
	...
	//
	access(all)
	resource Collection: PickemCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return PickemNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{PickemCollectionPublic}>(self.CollectionStoragePath)
