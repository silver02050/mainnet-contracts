...
access(all)
contract ARTIFACTPackV2: NonFungibleToken{ 
	// -----------------------------------------------------------------------
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			...
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: ARTIFACTPackV2.collectionStoragePath, publicPath: ARTIFACTPackV2.collectionPublicPath, publicCollection: Type<&ARTIFACTPackV2.Collection>(), publicLinkedType: Type<&ARTIFACTPackV2.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-ARTIFACTPackV2.createEmptyCollection(nftType: Type<@ARTIFACTPackV2.Collection>())
						})
	...
	resource interface CollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrow(id: UInt64): &ARTIFACTPackV2.NFT?
	}
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		fun withdraw(withdrawID: UInt64): @{NonFungibleToken.NFT}{ 
			...
			let token <- self.ownedNFTs.remove(key: withdrawID) ?? panic("Cannot withdraw: ARTIFACTPackV2 does not exist in the collection")
	...
	//
	access(account)
	fun createPackTemplate(metadata:{ String: String}, totalSupply: UInt64, maxQuantityPerTransaction: UInt64, packsAvailable: [PackOption]): PackTemplate{ 
	...
	access(account)
	fun getTemplateIdsFromPacksAvailable(packTemplate: ARTIFACTPackV2.PackTemplate): ARTIFACTPackV2.PackOption{ 
		pre{ 
			packTemplate.packsAvailable.length > 0:
				"No pack available"
		}
