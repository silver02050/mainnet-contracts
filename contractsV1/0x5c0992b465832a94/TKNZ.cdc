...
access(all)
contract TKNZ: NonFungibleToken{ 
	// -----------------------------------------------------------------------
	...
	access(all)
	event DropCreated(dropID: UInt32, series: UInt32)
	
	// Emitted when a new Play is added to a Drop
	access(all)
	event PlayAddedToDrop(dropID: UInt32, playID: UInt32)
	
	// Emitted when a Play is retired from a Drop and cannot be used to mint
	access(all)
	event PlayRetiredFromDrop(dropID: UInt32, playID: UInt32, numNFTs: UInt32)
	
	// Emitted when a Drop is locked, meaning Plays cannot be added
	access(all)
	event DropLocked(dropID: UInt32)
	
	// Emitted when a NFT is minted from a Drop
	access(all)
	event NFTMinted(NFTID: UInt64, playID: UInt32, dropID: UInt32, serialNumber: UInt32)
	...
	resource Drop{ 
		...
		fun addPlay(playID: UInt32){ 
			pre{ 
				TKNZ.playDatas[playID] != nil:
					"Cannot add the Play to Drop: Play doesn't exist."
				!self.locked:
					"Cannot add the play to the Drop after the drop has been locked."
				self.numberMintedPerPlay[playID] == nil:
					"The play has already beed added to the drop."
			}
		...
		fun retirePlay(playID: UInt32){ 
			pre{ 
				self.retired[playID] != nil:
					"Cannot retire the Play: Play doesn't exist in this drop!"
			}
		...
		fun mintNFT(playID: UInt32): @NFT{ 
			pre{ 
				self.retired[playID] != nil:
					"Cannot mint the NFT: This play doesn't exist."
				!self.retired[playID]!:
					"Cannot mint the NFT from this play: This play has been retired."
			}
			...
			// Mint the new NFT
			let newNFT: @NFT <- create NFT(serialNumber: numInPlay + UInt32(1), playID: playID, dropID: self.dropID)
		...
		init(dropID: UInt32){ 
			pre{ 
				TKNZ.drops[dropID] != nil:
					"The drop with the provided ID does not exist"
			}
	...
	resource Admin{ 
		...
		fun borrowDrop(dropID: UInt32): &Drop{ 
			pre{ 
				TKNZ.drops[dropID] != nil:
					"Cannot borrow Drop: The Drop doesn't exist"
			}
	...
	init(){ 
		...
		self.account.storage.save<@Collection>(<-create Collection(), to: /storage/NFTCollection)
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{NFTCollectionPublic}>(/storage/NFTCollection)
		self.account.capabilities.publish(capability_1, at: /public/NFTCollection)
		// Put the Minter in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: /storage/TKNZAdmin)
