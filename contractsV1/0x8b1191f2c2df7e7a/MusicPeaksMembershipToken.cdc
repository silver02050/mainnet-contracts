...
// constraint on when users can claim a MusicPeaksMembershipToken. They would do that by passing in
...
access(all)
contract MusicPeaksMembershipToken: NonFungibleToken{ 
	/***********************************************/
	/******************** PATHS ********************/
	/***********************************************/
	access(all)
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: MusicPeaksMembershipToken.FLOATCollectionStoragePath, publicPath: MusicPeaksMembershipToken.FLOATCollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-MusicPeaksMembershipToken.createEmptyCollection(nftType: Type<@MusicPeaksMembershipToken.Collection>())
						})
		...
		init(_eventDescription: String, _eventHost: Address, _eventId: UInt64, _eventImage: String, _eventName: String, _originalRecipient: Address, _serial: UInt64){ 
			...
			self.eventsCap = getAccount(_eventHost).capabilities.get<&FLOATEvents>(MusicPeaksMembershipToken.FLOATEventsPublicPath)!
			...
			MusicPeaksMembershipToken.totalSupply = MusicPeaksMembershipToken.totalSupply + 1
		}
	}
	...
	resource FLOATEvent: FLOATEventPublic, ViewResolver.Resolver{ 
		...
		// that MusicPeaksMembershipToken. Must be serial --> TokenIdentifier because
		...
		fun getCurrentHolder(serial: UInt64): TokenIdentifier?{ 
			...
			let collection = getAccount(data.address).capabilities.get<&Collection>(MusicPeaksMembershipToken.FLOATCollectionPublicPath).borrow<&Collection>()
		...
		fun verifyAndMint(recipient: &Collection, params:{ String: AnyStruct}): UInt64{ 
			...
			for identifier in self.verifiers.keys{ 
				...
				while i < typedModules.length{ 
					let verifier = typedModules[i] as &{MusicPeaksMembershipToken.IVerifier}
		...
		fun claim(recipient: &Collection, params:{ String: AnyStruct}){ 
			pre{ 
				self.getPrices() == nil:
					"You need to purchase this MusicPeaksMembershipToken."
				self.claimed[(recipient.owner!).address] == nil:
					"This person already claimed their FLOAT!"
				self.claimable:
		...
		init(_claimable: Bool, _description: String, _extraMetadata:{ String: AnyStruct}, _host: Address, _image: String, _name: String, _transferrable: Bool, _url: String, _verifiers:{ String: [{IVerifier}]}){ 
			...
			MusicPeaksMembershipToken.totalFLOATEvents = MusicPeaksMembershipToken.totalFLOATEvents + 1
	...
	resource FLOATEvents: FLOATEventsPublic, ViewResolver.ResolverCollection{ 
		...
		fun borrowSharedRef(fromHost: Address): &FLOATEvents{ 
			...
			let otherFLOATEvents = getAccount(fromHost).capabilities.get<&FLOATEvents>(MusicPeaksMembershipToken.FLOATEventsPublicPath).borrow<&FLOATEvents>() ?? panic("Could not borrow the public FLOATEvents.")
	...
	init(){ 
		...
		self.FLOATCollectionStoragePath = /storage/MusicPeaksMembershipTokenCollectionStoragePath
		self.FLOATCollectionPublicPath = /public/MusicPeaksMembershipTokenCollectionPublicPath
		self.FLOATEventsStoragePath = /storage/MusicPeaksMembershipTokenEventsStoragePath
		self.FLOATEventsPrivatePath = /private/MusicPeaksMembershipTokenEventsPrivatePath
		self.FLOATEventsPublicPath = /public/MusicPeaksMembershipTokenEventsPublicPath
	}
}
