...
access(all)
contract StanzClub: NonFungibleToken{ 
	access(all)
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.ExternalURL>():
					return MetadataViews.ExternalURL(self.link.concat(self.batch.toString()).concat("/").concat(self.sequence.toString()))
				// This view provides information for where to set up a NFT 
				// collection in storage and to create the empty collection.
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: StanzClub.CollectionStoragePath, publicPath: StanzClub.CollectionPublicPath, publicCollection: Type<&StanzClub.Collection>(), publicLinkedType: Type<&StanzClub.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-StanzClub.createEmptyCollection(nftType: Type<@StanzClub.Collection>())
						})
				...
				case Type<MetadataViews.NFTCollectionDisplay>():
					...
					if var _file = self.metadata["SQUARE_IMAGE_FILE"]{ 
						squareImageFile = _file
					}
					if var _type = self.metadata["SQUARE_IMAGE_TYPE"]{ 
					...
					if var _file = self.metadata["BANNER_IMAGE_FILE"]{ 
						bannerImageFile = _file
					}
					if var _type = self.metadata["BANNER_IMAGE_TYPE"]{ 
					...
					return MetadataViews.NFTCollectionDisplay(name: "StanzClub Collection", description: "StanzClub Collection", externalURL: MetadataViews.ExternalURL(self.link), squareImage: squareImage, bannerImage: bannerImage, socials:{} )
				// This view provides the distinct NFT traits information. 
				case Type<MetadataViews.Traits>():
					let traits: [MetadataViews.Trait] = []
					for key in self.metadata.keys{ 
						traits.append(MetadataViews.Trait(name: key, value: self.metadata[key], displayType: key, rarity: MetadataViews.Rarity(score: 0.0 as UFix64, max: 100.0 as UFix64, description: key)))
					}
	...
	access(all)
	resource Collection: StanzClubCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{NonFungibleToken.CollectionPublic, StanzClub.StanzClubCollectionPublic, ViewResolver.ResolverCollection}>(self.CollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: self.CollectionPublicPath)
		self.account.storage.save(<-create NFTMinter(), to: self.MinterStoragePath)
		self.account.storage.save(<-create Modifier(), to: /storage/StanzClubModifier)
