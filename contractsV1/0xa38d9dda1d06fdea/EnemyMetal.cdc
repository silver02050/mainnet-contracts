...
//
access(all)
contract EnemyMetal: NonFungibleToken{ 
	// Events
	//
	access(all)
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<MetadataViews.Display>():
					return MetadataViews.Display(name: self.data.metadata, description: "", thumbnail: MetadataViews.HTTPFile(url: "https://api.enemymetal.com/api/v1/nfts/thumbnail?metadata_hash=".concat(self.data.metadata)))
			}
			return nil
		}
	...
	access(all)
	resource interface EnemyMetalCollectionPublic{ 
		access(all)
		...
		fun borrowEnemyMetalNFT(id: UInt64): &EnemyMetal.NFT?{ 
	...
	//
	access(all)
	resource Collection: EnemyMetalCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
