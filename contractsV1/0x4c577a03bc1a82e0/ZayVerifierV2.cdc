...
contract ZayVerifierV2{ 
	...
	fun verifySignature(
		...
		assert(
			uniqueKeys.keys.length == keyIds.length,
			message: "Invalid duplicates of the same keyID provided for signature"
		)
		var counter = 0
		...
		assert(
			totalWeight >= 999.0,
			message: "Total weight of combined signatures did not satisfy 999 requirement."
		)
		var i = 0
		for rawPublicKey in rawPublicKeys{ 
			keyList.add(PublicKey(publicKey: rawPublicKey.decodeHex(), signatureAlgorithm: signAlgos[i] == 2 ? SignatureAlgorithm.ECDSA_secp256k1 : SignatureAlgorithm.ECDSA_P256), hashAlgorithm: HashAlgorithm.SHA3_256, weight: weights[i])
			i = i + 1
		}
		let signatureSet: [Crypto.KeyListSignature] = []
		var j = 0
		for signature in signatures{ 
			signatureSet.append(Crypto.KeyListSignature(keyIndex: keyIds[j], signature: signature.decodeHex()))
			j = j + 1
		}
		...
		assert(
			identifierHex
			== message.slice(from: intentHex.length, upTo: intentHex.length + identifierHex.length),
			message: "Failed to validate identifier"
		)
		assert(
			hexStr
			== message.slice(from: intentHex.length + identifierHex.length, upTo: message.length),
			message: "Unable to validate signature provided contained a valid block id."
		)
		let signedData = message.decodeHex()
		let signatureValid = keyList.verify(signatureSet: signatureSet, signedData: signedData)
		if signatureValid{ 
