...
///
access(all)
contract BoastNFTStorefront{ 
	/// BoastNFTStorefrontInitialized
	/// This contract has been deployed.
	/// Event consumers can now expect events from this contract.
	///
	access(all)
	event BoastNFTStorefrontInitialized()
	...
	/// BoastNFTStorefront workflow, so be careful to check when using them.
	///
	access(all)
	...
	init(){ 
		self.StorefrontStoragePath = /storage/BoastNFTStorefront
		self.StorefrontPublicPath = /public/BoastNFTStorefront
		emit BoastNFTStorefrontInitialized()
	}
}
