...
contract PublishedNFTStorefront{ 
	...
	init(){ 
		...
		self.account.storage.save(
			<-PublishedNFTStorefront.createStorefront(),
			to: self.StorefrontStoragePath
		)
		var capability_1 =
			self.account.capabilities.storage.issue<&PublishedNFTStorefront.Storefront>(
