...
///
access(all)
contract GroundNFTStorefront{ 
	/// GroundNFTStorefrontInitialized
	/// This contract has been deployed.
	/// Event consumers can now expect events from this contract.
	///
	access(all)
	event GroundNFTStorefrontInitialized()
	...
	/// GroundNFTStorefront workflow, so be careful to check when using them.
	///
	access(all)
	...
	init(){ 
		self.StorefrontStoragePath = /storage/GroundNFTStorefront
		self.StorefrontPublicPath = /public/GroundNFTStorefront
		emit GroundNFTStorefrontInitialized()
	}
}
