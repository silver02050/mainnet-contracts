...
contract CompoundInterest{ 
	access(self)
	let magic_factors: [[UFix64; 10]; 8]
	
	access(self)
	let powers_of_10: [UFix64; 12]
	
	access(self)
	let ks: [UFix64; 101]
	
	access(all)
	let k15: UFix64 // k for 15% APY
	
	
	access(all)
	let k100: UFix64 // k for 100% APY
	
	
	access(all)
	let k200: UFix64 // k for 200% APY
	
	
	access(all)
	let k2000: UFix64 // k for 2000% APY
	...
	fun pow10(_ x: UFix64): UFix64{ 
		pre{ 
			...
			x <= 11.26591972:
				"Cannot exceed Cadence UFix64 limit 184467440737.09551615"
		}
		...
		return self.powers_of_10[x_integer] * self.magic_factors[0][f1] * self.magic_factors[1][f2]
