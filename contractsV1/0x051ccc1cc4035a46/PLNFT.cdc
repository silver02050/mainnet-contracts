...
access(all)
contract PLNFT: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: PLNFT.CollectionStoragePath, publicPath: PLNFT.CollectionPublicPath, publicCollection: Type<&PLNFT.Collection>(), publicLinkedType: Type<&PLNFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-PLNFT.createEmptyCollection(nftType: Type<@PLNFT.Collection>())
						})
	...
	///
	access(all)
	resource interface PLNFTCollectionPublic{ 
		access(all)
		...
		fun borrowPLNFT(id: UInt64): &PLNFT.NFT?{ 
			post{ 
	...
	///
	access(all)
	resource Collection: PLNFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return PLNFT as &{ViewResolver.Resolver}
		}
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String, royalties: [MetadataViews.Royalty]){ 
			...
			metadata["foo"] = "bar"
			// create a new NFT
			var newNFT <- create NFT(id: PLNFT.totalSupply, name: name, description: description, thumbnail: thumbnail, royalties: royalties, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&PLNFT.Collection>(self.CollectionStoragePath)
