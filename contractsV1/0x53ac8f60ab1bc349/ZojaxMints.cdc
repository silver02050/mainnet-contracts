...
access(all)
contract ZojaxMints: NonFungibleToken, ViewResolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: ZojaxMints.CollectionStoragePath, publicPath: ZojaxMints.CollectionPublicPath, publicCollection: Type<&ZojaxMints.Collection>(), publicLinkedType: Type<&ZojaxMints.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-ZojaxMints.createEmptyCollection(nftType: Type<@ZojaxMints.Collection>())
						})
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String, royalties: [MetadataViews.Royalty]){ 
			...
			metadata["foo"] = "bar"
			// create a new NFT
			var newNFT <- create NFT(id: ZojaxMints.totalSupply, name: name, description: description, thumbnail: thumbnail, royalties: royalties, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&ZojaxMints.Collection>(self.CollectionStoragePath)
