...
contract FridgeMagnetV1: NonFungibleToken{ 
	...
	access(all)
	event SetUpdated(setID: UInt32, name: String)
	...
	struct SetData{ 
		...
		init(setID: UInt32, name: String, description: String, image: String, metadata:{ String: AnyStruct}){ 
			pre{ 
				name.length > 0:
	...
	resource Set{ 
		...
		init(name: String, description: String, image: String, metadata:{ String: AnyStruct}){ 
			...
			FridgeMagnetV1.setDatas[self.setID] = SetData(setID: self.setID, name: name, description: description, image: image, metadata: metadata)
		}
		...
		fun updateDescription(newDescription: String){ 
			...
			let oldSetData = FridgeMagnetV1.setDatas[self.setID]!
			FridgeMagnetV1.setDatas[self.setID] = SetData(setID: self.setID, name: oldSetData.name, description: newDescription, image: oldSetData.image, metadata: oldSetData.getMetadata())
		...
		fun updateImage(newImage: String){ 
			...
			let oldSetData = FridgeMagnetV1.setDatas[self.setID]!
			FridgeMagnetV1.setDatas[self.setID] = SetData(setID: self.setID, name: oldSetData.name, description: oldSetData.description, image: newImage, metadata: oldSetData.getMetadata())
			emit SetUpdated(setID: self.setID, name: oldSetData.name)
		}
		...
		fun updateMetadata(newMetadata:{ String: AnyStruct}){ 
			pre{ 
				newMetadata.length != 0:
					"New Metadata cannot be empty!!!"
			}
			let oldSetData = FridgeMagnetV1.setDatas[self.setID]!
			FridgeMagnetV1.setDatas[self.setID] = SetData(setID: self.setID, name: oldSetData.name, description: oldSetData.description, image: oldSetData.image, metadata: newMetadata)
			emit SetUpdated(setID: self.setID, name: oldSetData.name)
		}
	}
	...
	// Updated one
	access(all)
	struct FridgeMagnetV1CustomViewsUpdated{ 
		// All the var we want to view
		access(all)
		let name: String
		...
		init(name: String, description: String, image: String, setID: UInt32, serialNumber: UInt32, totalNFTInSet: UInt32?, externalUrl: String?, location: String?, businessName: String?, type: String?, additionalData: String?, mintingDate: UFix64){ 
			self.name = name
			self.description = description
			self.image = image
			self.setID = setID
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		view fun getViews(): [Type]{ 
			return [Type<MetadataViews.Display>(), Type<MetadataViews.Royalties>(), Type<MetadataViews.ExternalURL>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.NFTCollectionDisplay>(), Type<MetadataViews.Serial>(), Type<FridgeMagnetV1CustomViews>(), Type<FridgeMagnetV1CustomViewsUpdated>()]
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.ExternalURL>():
					...
					return MetadataViews.ExternalURL("https://www.fridgemagnet.xyz/nft/")
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: FridgeMagnetV1.CollectionStoragePath, publicPath: FridgeMagnetV1.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-FridgeMagnetV1.createEmptyCollection(nftType: Type<@FridgeMagnetV1.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					...
					return MetadataViews.NFTCollectionDisplay(name: "FridgeMagnet", description: "FridgeMagnet makes managing NFT campaigns easy and accessible for businesses.", externalURL: MetadataViews.ExternalURL("https://www.fridgemagnet.xyz/nft"), squareImage: squareMedia, bannerImage: bannerMedia, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/FridgemagnetXYZ")})
				...
				case Type<FridgeMagnetV1CustomViewsUpdated>():
					let externalUrl = self.getSetMetadataByField(field: "externalUrl") ?? ""
					let location = self.getSetMetadataByField(field: "location") ?? ""
					let businessName = self.getSetMetadataByField(field: "businessName") ?? ""
					let type = self.getSetMetadataByField(field: "type") ?? ""
					let additionalData = self.getSetMetadataByField(field: "additionalData") ?? ""
					return FridgeMagnetV1CustomViewsUpdated(name: self.name(), description: self.description(), image: self.image(), setID: self.data.setID, serialNumber: self.data.serialNumber, totalNFTInSet: FridgeMagnetV1.getTotalNFTInSet(setID: self.data.setID), externalUrl: externalUrl as? String, location: location as? String, businessName: businessName as? String, type: type as? String, additionalData: additionalData as? String, mintingDate: self.data.mintingDate)
			}
			return nil
		}
